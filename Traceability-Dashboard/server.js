...
// 1. DATABASE CONFIGURATION (Update with your SQL credentials)
const dbConfig = {
  host: 'localhost',
  user: 'root',
  password: 'Admin@123',
  database: 'factory_defects_db'
};

// 2. LIVE IN-MEMORY TALLY COUNTS (Resets/clears after successful hourly database saves)
let liveCounts = {
    channel1: {},
    channel2: {},
    channel3: {}
...
app.post('/api/counts/:id/increment', (req, res) => {
  console.log("postapicall",req.query)
    const { id } = req.params;
    const channel = req.query.channel || 'channel1';
...
// 4. EVERY 1 HOUR: COMMIT SNAPSHOTS TO SQL DATABASE AUTOMATICALLY
async function saveDataToSQL() {
    ...
    try {
        ...
        for (const channel of ['channel1', 'channel2', 'channel3']) {
            ...
            // Clear current working tally for the next hour block tracking
...
// Set up interval trigger loop (1 Hour = 3600000 milliseconds)
